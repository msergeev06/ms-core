...
class nf_pp
{
	...
	protected function p_array ($val, $curLevel)
	{
		$size = sizeof ($val);
		echo
			'<span class="pp_array pp_value">Array</span><i class="pp_ctrl pp_ctrlCollapseCh" title="Fold/unfold children">('
			.$size.')</i></div>';
		echo '<ul class="pp_container">';
		if ($size)
		{
	...
	protected function p_object ($val, $curLevel)
	{
		...
		$val = (array)$val;
		$size = sizeof ($val);
		echo '<span class="pp_object pp_value">Object &lt;'.$className
			.'&gt;</span><i class="pp_ctrl pp_ctrlCollapseCh" title="Fold/unfold children">('.$size.')</i></div>';
		echo '<ul class="pp_container">';
		if (!in_array ($val, $this->arRecursion, true))
		{ // check for recursion
			if ($size)
			{
	...
	protected function backtrace ()
	{
		...
		if ($backtrace[2]['function'] == 'msDebug' || $backtrace[2]['function'] == 'msDebugDie')
		{
			$arToPrint = $backtrace[2 + self::TRACE_DEPTH];
		} // run as a function
		else
		{
			$arToPrint = $backtrace[1 + self::TRACE_DEPTH];
		} // run as a method
		echo '<div class="pp_mark">'.$arToPrint['file'].' <span title="Line number">'.$arToPrint['line']
			.'</span></div>';
	}
